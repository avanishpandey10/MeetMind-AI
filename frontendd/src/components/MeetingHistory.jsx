...
const MeetingHistory = ({ setCurrentMeeting }) => {
  ...
  const getCompletionPercentage = (meeting) => {
    if (!meeting.actionItems?.length) return 0
    const completed = meeting.actionItems.filter(i => i.status === 'completed').length
                    ...
                    <div className="flex flex-wrap items-center gap-4 mt-3">
                      ...
                      <span className="text-xs text-gray-500 flex items-center space-x-1">
                        ...
                        <span>
                          ...
                          {meeting.actionItems?.length || 0} completed
                        </span>
                      </span>
                      <span className="text-xs text-gray-500 flex items-center space-x-1">
                        <span>💡</span>
                      ...
                      {getCompletionPercentage(meeting) > 0 && (
                        <div className="flex items-center space-x-2">
                          <div className="w-20 bg-gray-200 rounded-full h-1.5">
                            <div 
                              className="bg-green-500 h-1.5 rounded-full"
                              style={{ width: `${getCompletionPercentage(meeting)}%` }}
                            ></div>
                          </div>
                          <span className="text-xs text-gray-500">
                            {getCompletionPercentage(meeting)}%
                          </span>
                        </div>
                      )}
                    </div>
            ...
            {pagination && pagination.pages > 1 && (
              <div className="flex justify-center items-center space-x-2 mt-6 pt-6 border-t border-gray-200">
                <button
                  ...
                  disabled={page === 1}
                  className="px-4 py-2 bg-gray-100 rounded-lg hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed transition-all font-medium text-sm"
                >
                  ← Previous
                </button>
